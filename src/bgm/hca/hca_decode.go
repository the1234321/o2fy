...
func (h *Hca) DecodeFromFile(src, dst string) int {
	f, err := os.Open(src)
	if err != nil {
		return RetFileFormatError
	}
	defer f.Close()
...
func (h *Hca) DecodeFromBytes(data []byte) (decoded []byte, ret int) {
	decodedData := []byte{}

	if len(data) < 8 {
		return decodedData, RetFileFormatError
	}
	...
	if ret != RetSuccess {
		return decodedData, ret
	}

	tempfile.Seek(0, 0)
	decodedData, _ = ioutil.ReadAll(tempfile)

	return decodedData, RetSuccess
}

func (h *Hca) decodeBuffer(r *endibuf.Reader, w *endibuf.Writer) int {
	saveEndian := r.Endian
